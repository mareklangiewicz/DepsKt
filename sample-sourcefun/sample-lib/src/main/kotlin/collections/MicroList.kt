...
class MicroMutableListSlice<T>(asrc: MicroMutableList<T>, astart: Int, astop: Int) : MicroMutableList<T>,
  MicroListSlice<T, MicroMutableList<T>>(asrc, astart, astop) {
  override fun set(idx: Int, item: T) {
    src[start + idx.pos(size).chk(0, size - 1)] = item
  }
}

operator fun <T> MicroList<T>.get(start: Int, stop: Int) = MicroListSlice(this, start, stop) // TODO LATER: test it
operator fun <T> MicroMutableList<T>.get(start: Int, stop: Int) =
  MicroMutableListSlice(this, start, stop) // TODO LATER: test it
...
class MicroMutableListConcat<T>(afirst: MicroMutableList<T>, asecond: MicroMutableList<T>) : MicroMutableList<T>,
  MicroListConcat<T, MicroMutableList<T>>(afirst, asecond) {
